...
class TestAccgramFilterSplitWlc(unittest.TestCase):
    def test_run_writes_native_provenance_json_sidecar(self):
        with TemporaryDirectory() as tmp_dir:
            ...
            troublemakers_out = (
                Path(tmp_dir) / "out" / "accgram" / "goerwitz" / "_troublemakers.json"
            )
            ...
            split_payload = json.loads(
                split_provenance_path.read_text(encoding="utf-8")
            )
            self.assertEqual(
            ...
            self.assertEqual(
                split_payload["artifacts_description"], "unfiltered WLC split outputs"
            )
            ...
            troublemakers_payload = json.loads(
                troublemakers_out.read_text(encoding="utf-8")
            )
            self.assertEqual(
            ...
            self.assertFalse(
                any(
                    item["ref"] == "ps 1:1"
                    for item in troublemakers_payload["troublemakers"]
                )
            )
    ...
    def test_run_rejects_same_output_directories(self):
        with TemporaryDirectory() as tmp_dir:
            ...
            with self.assertRaisesRegex(ValueError, "must be different directories"):
                filter_split_wlc.run(
                    args, lambda input_path, out_dir, keep_line_fn=None: None
                )

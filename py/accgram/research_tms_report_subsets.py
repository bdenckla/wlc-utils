...
def build_msp_yes_related_pages_top_contents(main_html_out_path: Path) -> tuple[object, ...]:
...
def build_msp_no_related_pages_top_contents(main_html_out_path: Path) -> tuple[object, ...]:
    main_name = main_html_out_path.name
    yes_name = missing_sof_pasuq_yes_html_out_path(main_html_out_path).name
    return (
        wlc_utils_html.heading_level_2("Related pages"),
...
def _row_is_missing_sof_pasuq_yes(row: dict[str, object]) -> bool:
    ...
    return any(_assessment_value_contains_missing_sof_pasuq_marker(value) for value in assessment.values())
...
def _assessment_value_contains_missing_sof_pasuq_marker(value: object) -> bool:
    ...
    if isinstance(value, list):
        return any(_assessment_value_contains_missing_sof_pasuq_marker(item) for item in value)

    if isinstance(value, dict):
        return any(_assessment_value_contains_missing_sof_pasuq_marker(item) for item in value.values())

# Plan: Run-goerwitz stderr JSON summary

Date: 2026-05-26

## Goal
Add a deterministic final-step JSON summary to run-goerwitz so each run produces an aggregate artifact for all out/accgram/goerwitz-stderr/*_ag.stderr.txt files.
...
- In scope: final-step summarization as part of run-goerwitz execution.
- Out of scope: redesign of run-goerwitz pipeline, historical dashboards, parser parity fixes.

## Implementation Steps
1. Phase 1: schema and output contract.
2. Add summary artifact path under the existing stderr directory. Recommended filename: out/accgram/goerwitz-stderr/_summary.stderr.json.
...
5. Phase 2: run-goerwitz integration.
6. In py/accgram/run_goerwitz.py, add a final-step helper invoked from run(args) immediately after run_goerwitz_binary(...) returns.
...
12. Implement verse extraction rules tuned to observed goerwitz stderr formats, including lines ending in Book Chapter:Verse (example: Obadiah 1:2).
...
17. Initial behavior should preserve existing run-goerwitz success behavior.
18. Exclude cross-run accumulation and non-JSON reporting from this task.
19. Phase 5: verification.
20. Run main_accgram run-goerwitz and verify summary file is regenerated under out/accgram/goerwitz-stderr.
...
23. Confirm main_0_mega integration remains unchanged except for one additional run-goerwitz stdout line.

## Relevant Files
- py/accgram/run_goerwitz.py
- py/main_accgram.py
- py/accgram/filter_split_wlc.py
- out/accgram/goerwitz-stderr/

## Verification Checklist
1. Execute main_accgram run-goerwitz and confirm completion with existing counters plus one summary output line.
2. Verify summary JSON exists at out/accgram/goerwitz-stderr/_summary.stderr.json and contains deterministic sorted sections.
3. Re-run run-goerwitz and confirm summary output is deterministic for unchanged inputs.
...
2. Add optional strict mode later only if needed, to fail run-goerwitz when summary generation fails.
